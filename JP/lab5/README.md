 ...
 **VPC** のコンソールに移動し、このハンズオンの最初に作った「 **VPC ID** 」と、この章の最初に CloudFormation で作成した「 **サブネット ID** 」をメモします。
 ...
 2. AWS マネジメントコンソールのサービス一覧から **Amazon Redshift** を選択し、 **[Redshift ダッシュボード]** 画面の左ペインの **[セキュリティ]** を選択します。
...
### Step1：Redshift への接続
 ...
 1. **[Redshift ダッシュボード]** の 左ペインから **[Query editor]** を選択します。
...
Redshift にデータをロードします。
 ...
 6. **[Redshiftダッシュボード]** に戻り、画面左ペインの **[クラスター]** を選択し、作成した Redshift クラスター名（例：handson-minilake-dwh）をクリックします。 **[IAM Roles]** の **[See IAM roles]** をクリックします。

 7. さきほど作成した IAM ロール「 **handson-minilake-dwh**（任意）」を選択し、 **[変更の適用]** をクリックすると、一度サブウインドウが閉じます。 **[クラスターステータス]** が **[変更中]** となっているため、 **[使用可能]** になるまで待ちます。
