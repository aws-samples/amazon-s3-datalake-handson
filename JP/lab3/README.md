 ...
 **[実行結果例]**
 ...
 5. 「 **/etc/init.d/td-agent** 」ファイル（「 **/etc/td-agent/td-agent.conf** 」とは異なるファイル）を開き、13行目辺りに以下の行を追加します。
...
　　 **Note：** 2020 年 7 月現在、新しいコンソール画面が表示されている場合、旧コンソール画面に変更し、手順を進めてください。
...
　　**Note：** ブラウザでポップアップブロックが走ったら、許可して、1つ前の手順からやり直して下さい。
 ...
 7. **[Amazon Elasticsearch Service への minidatalake_group のストリーミングを開始する]** 画面において、そのまま **[次へ]** をクリックします。

 8. **[ログの形式]** において、 **[その他]** を選択し、**[次へ]** をクリックし、その後も **[次へ]** をクリックし、**[ストリーミングの開始]** をクリックします。


### Step2：Elasticsearch Service の設定

 1. **Kibana** の画面を開き、 **Kibana** の画面の左ペインから **[Management]** をクリックし、 **[Index Patterns]** をクリックします。  
 ...
 5. **Kibana** の画面の左ペインから **[Discover]** をクリックし、その右の Index を選ぶ箇所で「 **cwl-*** 」を選択します。値が取得されグラフが表示されていればOKです。
...
### Step3：CloudWatch アラームの設定
 ...
 1. AWS マネジメントコンソールのサービス一覧から **CloudWatch** を選択し、**[ロググループ]** をクリックし、「 **minilake_group**（任意）」のロググループにチェックを入れ、 **[メトリクスフィルタの作成]** をクリックします。

 2. フィルタパターンに「 **ERROR** 」を入力し、 **[パターンのテスト]** をクリックすることで内容を確認し、画面右下の **[メトリクスの割り当て]** をクリックします。  

 3. **[メトリクス名]** を「 **minilake_errlog**（任意）」と入力し、 **[フィルタの作成]** をクリックします。
 ...
 10. **[CloudWatch]** の **[アラーム]** をクリックし、グラフ画面を確認します。最初は状態が **[データ不足]** と表示されていますが、しばらくすると **[OK]** になります。1分間で50件以上  **ERROR** が発生するとアラートが上がります。10分ごとに ERROR が300件出る設定になっている為、10分毎にアラートが上がります。
